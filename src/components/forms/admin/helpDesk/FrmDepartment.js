...
import {
  ...
  PostNewDept,
  UpdateDept,
} from '../../../../store/slices/admin/helpDesk/DepartmentSlices';
...
const FrmDepartment = () => {
  ...
  const { data: DepartmentList } = useSelector((state) => state.departmentSlices);
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      if (selectedLocation) {
        dispatch(UpdateDept(values));
      } else {
        dispatch(PostNewDept(values));
      }
      formik.resetForm();
