...
import {
  ...
  PostNewIncident,
  UpdateIncident,
} from '../../../../store/slices/admin/helpDesk/IncidentSlices';
...
const FrmIncident = () => {
  ...
  const { data: IncidentList } = useSelector((state) => state.helpDeskIncidentSlices);
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      if (selectedBrand) {
        dispatch(UpdateIncident(values));
      } else {
        dispatch(PostNewIncident(values));
      }
      formik.resetForm();
  ...
  const handleRowDelete = () => {
    dispatch(DeleteIncident({ id: selectedBrand.Id }));
