...
import {
  ...
  PostNewActivity,
  UpdateActivity,
} from '../../../../store/slices/admin/helpDesk/ActivitySlices';
...
const FrmActivity = () => {
  ...
  const { data: HelpDeskActivityList } = useSelector((state) => state.helpDesActivitySlices);
  const [selectedBrand, setSelectedActivity] = useState(null);
  useEffect(() => {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      if (selectedBrand) {
        ...
        dispatch(PostNewActivity(values));
      }
      formik.resetForm();
  ...
  //   dispatch(DeleteActivity({ id: selectedBrand.Id }));
  // };
